...
const main = async () => {
  ...
  const liquiditydesired = ethers.parseUnits("1000", 18)
  ...
  const approvePool = await pool_contract.connect(signer).approve(router_address, liquiditydesired)
  await approvePool.wait()

  const removeLiquidity = await router_contract.connect(signer).removeLiquidity(usdc_address, dai_address, pool_balance, usdcmin, daimin, impersonated_address, deadline);
  ...
  console.log("After Remove USDC Balance:", ethers.formatUnits(new_usdc_balance, 6))
  console.log("After Remove DAI balance:", ethers.formatUnits(new_dai_balance,18))
  console.log("After Remove Pool Balance:", ethers.formatUnits(new_pool_balance, 18))
}
